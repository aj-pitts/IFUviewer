...
class InteractiveIFUViewer:
    def __init__(self, fluxcube: np.ndarray, modelcube: np.ndarray, wavelength:np.ndarray, datadict: Dict[str, dict], binmap:np.ndarray, 
                 redshiftmap: np.ndarray, MCMC_cube: Optional[np.ndarray] = None):
        
        self._validate_inputs(fluxcube, wavelength, datadict, binmap, redshiftmap, MCMC_cube)
    ...
    def _validate_inputs(self, fluxcube, wavelength, datadict, binmap, redshiftmap, MCMC_cube):
        if fluxcube.ndim != 3:
        ...
        for key, subdict in datadict.items():
            if not isinstance(subdict, dict):
                raise ValueError("Datadict must contain dictionaries as values")
            if subdict['image'].ndim != 2:
                raise ValueError(f"Image data under {key} is not 2D")
            if subdict['image'].shape != subdict['mask'].shape:
                raise ValueError(f"Image data of {key} has shape {subdict['image'].shape} while corresponding mask has shape {subdict['mask'].shape}")
            if subdict['image'].shape != (fluxcube.shape[1], fluxcube.shape[2]):
                raise ValueError(f"Image data of {key} does not match the spatial dimensions of the flux datacube")
...
def main_viewer(galname, bin_method):
    print(f"Launching interactive viewer for {galname} {bin_method}")
    print("\nInstructions:")
    ...
    #mcmc_cube = local['MCMC_RESULTS'].data

    keywords = ['EW_NOEM', 'SFRSD', 'V_NAI']

    datadict = {}
    for key in keywords:
        datamap = local[key].data
        datamask = local[f"{key}_MASK"].data
        kwargs_dict = get_imshow_kwargs(key)
        image_dict = dict(image=datamap, mask=datamask)
        merged = image_dict | kwargs_dict
        datadict[key] = merged
...
if __name__ == "__main__":
    args = arguments()
    print(f"Setting up viewer for {args.galname} {args.bin_method}")
    plt.style.use(util.defaults.matplotlib_rc())


    viewer = main_viewer(args.galname, args.bin_method)